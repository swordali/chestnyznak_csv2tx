...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            st.success(f"Başarıyla Yüklendi: `{uploaded_file.name}` ({toplam_veri} satır veri bulundu)")
            ...
            txt_data = yeni_df.to_csv(sep='\t', index=False, header=False, encoding=kodlama)
...
with tab2:
    ...
    if media_file is not None:
        ...
        with st.spinner("zxing-cpp motoru barkodları tarıyor..."):
            try:
                # Eğer yüklenen dosya PDF ise (PyMuPDF ile sayfa sayfa görsele çevir)
                if m_uzanti.lower() == ".pdf":
                    ...
                    for sayfa_no in range(len(doc)):
                        ...
                        pix = page.get_pixmap(dpi=300) # Netlik için 300 DPI
                        img = Image.frombytes("RGB", [pix.width, pix.height], pix.samples)
                        
                        # zxing-cpp ile sayfadaki tüm barkodları oku
                        results = zxingcpp.read_barcodes(img)
                        for res in results:
                            if res.text:
                                barkodlar.append(res.text)
                
                # Eğer doğrudan görsel ise (Pillow ile aç)
                else:
                ...
                if barkodlar:
                    ...
                    txt_data_b = df_b.to_csv(sep='\t', index=False, header=False, encoding=kodlama_b)
                    ...
                    st.warning("Belgede veya görselde okunabilir hiçbir DataMatrix barkodu bulunamadı. Görsel çözünürlüğünü kontrol edin.")
